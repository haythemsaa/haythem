...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-hard-hat"></i> Nouvel EPI</h2>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('ppe.store') }}" method="POST">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('equipment_type') is-invalid @enderror" id="equipment_type" name="equipment_type" required>
                                ...
                                @foreach($equipmentTypes as $key => $label)
                                    <option value="{{ $key }}" {{ old('equipment_type') == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('size') is-invalid @enderror" id="size" name="size" value="{{ old('size') }}" placeholder="S, M, L, 42, etc.">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('replacement_frequency_months') is-invalid @enderror" id="replacement_frequency_months" name="replacement_frequency_months" value="{{ old('replacement_frequency_months') }}" min="1" max="120">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                @foreach($statuses as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', 'in_use') == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                ...
                <div class="d-flex justify-content-end mt-4">
                    <a href="{{ route('ppe.index') }}" class="btn btn-secondary me-2">Annuler</a>
